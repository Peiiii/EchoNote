import {
  AIChatMessage,
  SpaceContext,
} from "@/desktop/features/demo/features/space-aware-chat/types";
...
export const useSpaceAIChat = (spaceId: string, spaceContext: SpaceContext) => {
  ...
  const generateAIResponse = async (userMessage: string): Promise<string> => {
    ...
    if (userMessage.includes("总结") || userMessage.includes("概括")) {
      return `基于该空间的 ${spaceContext.totalCount} 条记录，我可以看到主要涉及：${spaceContext.records
        .map(r => r.tags?.[0])
        .filter(Boolean)
        .join("、")} 等领域。需要我帮你分析某个具体方面吗？`;
    }

    if (userMessage.includes("进度") || userMessage.includes("状态")) {
      const workRecords = spaceContext.records.filter(
        r => r.tags?.includes("进度") || r.tags?.includes("完成")
      );
    ...
    if (userMessage.includes("计划") || userMessage.includes("安排")) {
      const planRecords = spaceContext.records.filter(
        r => r.tags?.includes("计划") || r.content.includes("需要") || r.content.includes("明天")
      );
