...
export function useAgentChatSync(conversationId: string, _channelId: string) {
  ...
  const updateMessage = useCallback(
    async (message: UIMessage) => {
      if (!userId) return;
      await firebaseAIConversationService.updateMessage(
        userId,
        conversationId,
        message.id,
        message
  ...
  const loadMoreMessages = useCallback(
    async (options?: MessageListOptions) => {
      if (!userId) return;
      setLoading(true);
      try {
        const moreMessages = await firebaseAIConversationService.listMessages(
          userId,
          conversationId,
          options
        );
