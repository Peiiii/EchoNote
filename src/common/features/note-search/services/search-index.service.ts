...
export class InMemorySearchIndex implements SearchIndex {
  ...
  indexNotes(notes: Note[]): void {
    ...
    for (const note of notes) {
      if (!note.isDeleted) {
        ...
        this.byChannelCount.set(note.channelId, (this.byChannelCount.get(note.channelId) || 0) + 1);
      }
    }
  }
  ...
  search(query: string, options?: SearchOptions): NoteSearchMatch[] {
    ...
    results.sort((a, b) => b.score - a.score || b.timestamp.getTime() - a.timestamp.getTime());
  ...
  private scoreNote(note: Note, query: string): NoteSearchMatch {
    ...
    const keywords = (note.aiAnalysis?.keywords || []).map((keyword: string) =>
      keyword.toLowerCase()
    );
