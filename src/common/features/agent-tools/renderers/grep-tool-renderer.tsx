...
export function GrepToolRenderer({ invocation }: GrepToolRendererProps) {
  ...
  const ParamPanel = ({ args, scope, effectiveIncludeCount }: { args: Partial<GrepToolArgs>; scope?: string; effectiveIncludeCount?: number }) => {
...
{cmd}
        ...
        <div className="text-xs text-gray-700 dark:text-gray-300">
          <span className="inline-block mr-2">scope: <code>{scope || 'global'}</code></span>
          {typeof effectiveIncludeCount === 'number' && (
            <span className="inline-block">channels: <code>{effectiveIncludeCount}</code></span>
          )}
        </div>
        <div className="text-[10px] text-gray-500 dark:text-gray-400">Scope is enforced by conversation/UI context and is not part of the grep command.</div>
      </div>
    );
  };
  ...
  return (
    ...
    >
      {(args, result, error) => {
        ...
        const paramPanel = <ParamPanel args={args} effectiveIncludeCount={result?.effectiveIncludeCount} />;
        if (error) {
          return (
            <div className="space-y-3">
              {paramPanel}
              <ErrorMessage error={error} fallbackMessage="An error occurred while searching" variant="alert" />
            </div>
          );
        }
        ...
        if (result.noteIds && result.noteIds.length > 0 && (!result.matches || result.matches.length === 0)) {
          return (
            <div className="space-y-3">
              {paramPanel}
        ...
        return (
          <div className="space-y-4">
            ...
            {result.matches.map((m) => (
              <div key={m.noteId} className="rounded-md border p-3 bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-800 space-y-2">
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  note: <code>{m.noteId}</code> • channel: <code>{m.channelId}</code> • {m.timestamp}
                </div>
                <div className="font-mono text-sm">
                  {m.contexts.map((c, idx) => (
                    <div key={idx} className="mb-3">
                      {c.before?.map((t, i) => (
                        <div key={`b-${i}`} className="text-gray-500 dark:text-gray-400">{t}</div>
                      ))}
                      <div className="bg-yellow-100 dark:bg-yellow-900/40 px-1 rounded">
                        {c.match}
                      </div>
                      {c.after?.map((t, i) => (
                        <div key={`a-${i}`} className="text-gray-500 dark:text-gray-400">{t}</div>
                      ))}
                    </div>
                  ))}
                </div>
