...
export function ToolDisplayHeader({
  ...
  return (
    <div className="px-2 flex gap-2 items-center overflow-hidden">
      <div className="flex items-center gap-2 flex-shrink-0">{icon}</div>
      <div className="overflow-y-auto flex-shrink-0">{title}</div>
      <div className="flex items-center gap-2 overflow-hidden">
        <div className="flex-shrink-0">{config.icon}</div>
        <span className={`text-sm font-medium truncate overflow-hidden ${config.color}`}>
          {statusText}
        </span>
      </div>
