 ...
 */
export const convertMessagesToUIMessages = (messages: Message[]): UIMessage[] => {
...
const mergeAssistantMessages = (params: {
  ...
  const toolCalls: ToolCall[] = messages
    .filter(
      (message): message is Message & { role: "assistant"; toolCalls: ToolCall[] } =>
...
export const convertMessageToMessages = (
  ...
  if (part.type === "text") {
    ...
    if (
      [ToolInvocationStatus.CALL, ToolInvocationStatus.PARTIAL_CALL].includes(
        part.toolInvocation.status
      )
    ) {
...
export const convertUIMessagesToMessages = (messages: UIMessage[]): Message[] => {
  return messages
    .filter(message => message.role !== "data")
    .map(message => {
      return message.parts
        .map((part, index) => {
          return convertMessageToMessages(
            message as UIMessage & { role: "system" | "assistant" | "user" },
            part,
            index
          );
        })
